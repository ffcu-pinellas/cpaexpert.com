    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-bold text-2xl text-slate-800 leading-tight">
                {{ __('Add Practice Area') }}
            </h2>
        </div>
    </x-slot>

    <div class="max-w-4xl">
        <form action="{{ route('practice-areas.store') }}" method="POST" class="space-y-6">
            @csrf
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        <label for="title" class="block text-sm font-semibold text-slate-700 mb-1">Area Title</label>
                        <input type="text" name="title" id="title" value="{{ old('title') }}" class="w-full rounded-lg border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 transition-all shadow-sm" placeholder="e.g. Criminal Defense" required>
                    ...
                    <div>
                        ...
                        <div class="flex items-center">
                            ...
                            <input type="text" name="slug" id="slug" value="{{ old('slug') }}" class="w-full rounded-r-lg border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 transition-all shadow-sm" placeholder="criminal-defense" required>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea name="icon" id="icon" rows="3" class="w-full rounded-lg border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 transition-all shadow-sm font-mono text-xs" placeholder='<svg class="w-6 h-6" ...'></textarea>
                    ...
                    <div>
                        ...
                        <textarea name="short_description" id="short_description" rows="3" class="w-full rounded-lg border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 transition-all shadow-sm" placeholder="A 2-3 sentence overview for the listing page">{{ old('short_description') }}</textarea>
                    </div>

                    <div>
                        <label for="content" class="block text-sm font-semibold text-slate-700 mb-2">Detailed Content</label>
                        <textarea name="content" id="content" rows="10" class="w-full rounded-lg border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 transition-all shadow-sm" placeholder="Explain this service in detail...">{{ old('content') }}</textarea>
                    </div>
                </div>
            </div>
